...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 25 10:27:58 2022
...
for i in range(N-1):
    ...
    for k in range(2):
        for j in range(2):
            if(j!=k):
                a[i,k,:] = -m[j] * (x[i,k,:] - x[i,j,:])/ ( np.linalg.norm((x[i,k,:] - x[i,j,:]))**3 )
...
ax.plot(x1_cmrf[:,0], x1_cmrf[:,1], color='green', label='body1' )
ax.plot(x2_cmrf[:,0], x2_cmrf[:,1], color='violet', label='body2' )
ax.xlabel('x')
ax.ylabel('y')
ax.legend()
fig.show()
